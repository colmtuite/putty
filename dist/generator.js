 ...
 */
export function resolveValue(value, property, theme) {
    ...
    if (value.startsWith('$')) {
        ...
        if (scale && theme.tokens[scale]) {
            const scaleTokens = theme.tokens[scale];
 ...
 */
export function generateRule(decl, theme) {
 ...
 */
export function parseStyleObject(styles, theme, parentSelector) {
    const declarations = [];
 ...
 */
export function generateCSS(rules, theme) {
    // Group by breakpoint
    const baseRules = [];
    const breakpointRules = new Map();
