 ...
 */
export type TokensForScale<T extends ThemeConfig, Scale extends keyof T['tokens']> = T['tokens'][Scale] extends Record<string, unknown> ? `$${string & keyof T['tokens'][Scale]}` : never;
/**
 * Get the allowed value type for a CSS property
 */
export type PropertyValue<T extends ThemeConfig, P extends keyof CSSProperties> = P extends keyof PropertyToScale ? PropertyToScale[P] extends keyof T['tokens'] ? TokensForScale<T, PropertyToScale[P]> | CSSProperties[P] : CSSProperties[P] : CSSProperties[P];
/**
 * Responsive value - either a plain value or an object with breakpoint keys
 */
export type ResponsiveValue<T extends ThemeConfig, V> = V | {
 ...
 */
export type CxFunction<T extends ThemeConfig> = (styles: StyleObject<T>) => CxResult;
